        ...
        if (xhttpNav.readyState == 4) {
            ...
            document.querySelectorAll('.sidenav a ,.topnav a').forEach(e => {
                e.addEventListener('click', click => {
                    ...
                    let pages = click.target.getAttribute('href').substr(1)
                    if (pages != '!') {
                        route(pages)
                    }
                })
    ...
    return new Promise((resolve, reject) => {
        xhttpPage.onreadystatechange = function () {
            ...
            if (xhttpPage.readyState == 4) {
                const dPage = document.querySelector('#content')
                if (xhttpPage.status == 200) {
                    dPage.innerHTML = xhttpPage.responseText
                    resolve(page)
                } else if (xhttpPage.status == 404) {
                    dPage.innerHTML = '<h2 class="center-align">Opps 404 : Page Not Found</h2>'
                    reject(new Error('http.status : 404 || Page not found'))
                } else {
                    dPage.innerHTML = '<h2 class="center-align">Owo.. Tidak bisa memuat halaman</h2>'
